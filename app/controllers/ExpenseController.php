...
class ExpenseController extends Controller {
    ...
    public function create() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->expense->create($data)) {
                header('Location: /ergon_clean/public/expenses?success=1');
                exit;
            } else {
    ...
    public function approve($id) {
        ...
        if (!$id) {
            header('Location: /ergon_clean/public/expenses?error=invalid_id');
            exit;
        }
        
        try {
            $this->expense->updateStatus($id, 'approved', $_SESSION['user_id']);
            header('Location: /ergon_clean/public/expenses?success=approved');
        } catch (Exception $e) {
            error_log('Expense approval error: ' . $e->getMessage());
            header('Location: /ergon_clean/public/expenses?error=approval_failed');
        }
        exit;
    }
    ...
    public function reject($id) {
        ...
        try {
            ...
            header('Location: /ergon_clean/public/expenses?success=rejected');
        } catch (Exception $e) {
            error_log('Expense rejection error: ' . $e->getMessage());
            header('Location: /ergon_clean/public/expenses?error=rejection_failed');
        }
        exit;
    }
}
