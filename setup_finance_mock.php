<?php
/**
 * Final Mock Data Solution
 * Creates working finance data for testing
 */
...
try {
    ...
    echo "Setting up finance tables and mock data...\n\n";
    
    // Drop and recreate tables to ensure correct structure
    $mysql->exec("DROP TABLE IF EXISTS finance_customers");
    $mysql->exec("DROP TABLE IF EXISTS finance_invoices");
    $mysql->exec("DROP TABLE IF EXISTS finance_purchase_orders");
    $mysql->exec("DROP TABLE IF EXISTS finance_payments");
    $mysql->exec("DROP TABLE IF EXISTS finance_quotations");
    
    // Create tables with correct structure
    $mysql->exec("CREATE TABLE finance_customers (
    ...
    $mysql->exec("CREATE TABLE finance_invoices (
    ...
    $mysql->exec("CREATE TABLE finance_purchase_orders (
    ...
    $mysql->exec("CREATE TABLE finance_payments (
    ...
    $mysql->exec("CREATE TABLE finance_quotations (
    ...
    echo "✅ Created all finance tables\n\n";
    ...
    $stmt = $mysql->prepare("INSERT INTO finance_customers (customer_id, customer_name, customer_gstin) VALUES (?, ?, ?)");
    ...
    $stmt = $mysql->prepare("INSERT INTO finance_invoices (invoice_number, customer_id, total_amount, taxable_amount, amount_paid, igst_amount, cgst_amount, sgst_amount, due_date, invoice_date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $stmt = $mysql->prepare("INSERT INTO finance_purchase_orders (po_number, customer_id, po_total_value, po_date, po_status) VALUES (?, ?, ?, ?, ?)");
    foreach ($pos as $po) {
        $stmt->execute($po);
    }
    ...
    $stmt = $mysql->prepare("INSERT INTO finance_payments (payment_id, customer_id, amount, payment_date, receipt_number, status) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    $stmt = $mysql->prepare("INSERT INTO finance_quotations (quotation_number, customer_id, quotation_amount, quotation_date, status) VALUES (?, ?, ?, ?, ?)");
    foreach ($quotations as $quotation) {
        $stmt->execute($quotation);
    }
    echo "✅ Inserted " . count($quotations) . " quotations\n";
    ...
    $totalRecords = count($customers) + count($invoices) + count($pos) + count($payments) + count($quotations);
    ...
    $stmt->execute(['mock_finance_data', $totalRecords, 'completed', 'Complete mock finance dataset created successfully']);
    
    echo "\n🎉 Complete finance mock data setup successful!\n";
    echo "📊 Summary:\n";
    echo "   - " . count($customers) . " customers\n";
    echo "   - " . count($invoices) . " invoices (₹4,25,000 total)\n";
    echo "   - " . count($pos) . " purchase orders (₹4,90,000 total)\n";
    echo "   - " . count($payments) . " payments (₹2,37,500 received)\n";
    echo "   - " . count($quotations) . " quotations (₹4,44,000 quoted)\n";
    echo "\n💡 Your finance module is now fully functional!\n";
    echo "🔄 When PostgreSQL connection is restored, the sync will replace this with real data.\n";
    echo "🌐 You can now test all finance features in the web interface.\n";
