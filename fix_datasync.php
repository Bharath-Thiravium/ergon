<?php
// One-time script to overwrite DataSyncService.php on server
...
class DataSyncService {
    ...
    public function syncAllTables() {
        if (!$this->isPostgreSQLAvailable()) {
            return array_fill_keys(
                ...
                [\'records\' => 0, \'status\' => \'unavailable\', \'error\' => \'pdo_pgsql driver not available\']
            );
        }
        return [
            \'customers\'      => $this->syncCustomers(),
            \'quotations\'     => $this->syncQuotations(),
            \'purchase_orders\'=> $this->syncPurchaseOrders(),
    ...
    private function syncTable($tableName, $selectQuery, $insertQuery, $fields) {
        ...
        try {
            ...
            $this->logSync($tableName, $recordsSynced, \'failed\', $e->getMessage(), $syncStarted);
            return [\'table\' => $tableName, \'records\' => $recordsSynced, \'status\' => \'error\', \'error\' => $e->getMessage()];
        }
    }
...
if (file_put_contents($target, $content)) {
    echo "✓ DataSyncService.php overwritten successfully.\n";
    echo "File size: " . filesize($target) . " bytes\n";
    echo "First 100 chars: " . substr(file_get_contents($target), 0, 100) . "\n";
    echo "\nDELETE this file now.\n";
} else {
    echo "❌ Failed to write file — check permissions on app/services/\n";
}
?>
